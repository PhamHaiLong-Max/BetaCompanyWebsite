...
<body>
    ...
    <?php
        ...
        {
            if(!empty($_POST["eoiid"]))
            {
                $id=sanitise_input($_POST["eoiid"]);
                $status=sanitise_input($_POST["status"]);
                switch($status){
                    case "New":
                        $query="UPDATE eoi SET status='New' WHERE EOInumber=$id;";
                        break;
                    case "Current":
                        $query="UPDATE eoi SET status='Current' WHERE EOInumber=$id;";
                        break;
                    case "Final":
                        $query="UPDATE eoi SET status='Final' WHERE EOInumber=$id;";
                        break;
                }
                ...
                {
                    echo "<p>No changes made since no result is found!</p>";
                }
                else
                {
